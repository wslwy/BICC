...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("PintSmplThresh",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("ls_counter",
                                "The receiver has received the counter of bytes for BiCC",
                                UintegerValue(0),
                                MakeUintegerAccessor(&RdmaHw::ls_counter),
                                MakeUintegerChecker<uint64_t>())
		;
	return tid;
}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	/** BiCC **/
//	std::cout << ch.udp.ih.nhop << std::endl;
	if(ch.udp.ih.nhop >= 8) {
		ls_counter += p->GetSize();
	}
	/** BiCC **/
	...
	if (x == 1 || x == 2){ //generate ACK or NACK
		...
		/** BiCC **/
		ch.udp.ih.recvBiCCBytes = ls_counter;
		/** BiCC **/
...
//	std::cout << "1、  " << ch.sip << " " << port << " " << qIndex << std::endl;
       ...
       // printf("%u %u %u %u\n", nowTime, qp->sip.Get(), qp->dip.Get(), nowCur);
        ...
        uint32_t bufferSizeUsed = 0;
        for(uint32_t i = 0; i < ih.nhop; i++){
                bufferSizeUsed += ih.hop[i].GetQlen();
        }
...
/*
	IntHeader &ih = ch.ack.ih;
        ...
        for(uint32_t i = 0; i < 5; i++){
                printf("%llu %llu %llu %d %lu\n", nowTime, qp->sip.Get(), qp->dip.Get(), i, ih.hop[i].GetQlen());
        }
*/

	printf("%llu %llu %llu %f %lu\n", nowTime, qp->sip.Get(), qp->dip.Get(), nowCur, bufferSizeUsed);
...
//		std::cout << "SendDCI Recv" << std::endl;
	        uint16_t qIndex = ch.bicc.qIndex;
//	        uint16_t port = ch.ack.dport;
		uint16_t port = ch.bicc.dport;
//		uint16_t port = ch.bicc.dport;
//	        std::cout << port << std::endl;	
//      	Ptr<RdmaQueuePair> qp = GetQp(ch.dip, port, qIndex);
//		Ptr<RdmaQueuePair> qp = GetQp(184558593, 10000, qIndex);
		Ptr<RdmaQueuePair> qp = GetQp(ch.sip, port, qIndex);
...
void RdmaHw::cnp_received_mlx(Ptr<RdmaQueuePair> q, bool isLongLoop){
//	std::cout << "&*&*&*" << std::endl;
//	std::cout << q << std::endl;
...
//	q->mlx.m_decrease_cnp_arrived = true; // set CNP_arrived bit for rate decrease
	...
	/** BiCC **/
/*	if(!isLongLoop){
		q->BiCC.m_decrease_rns_arrived = true;
	}else{
...
void RdmaHw::CheckRateDecreaseMlx(Ptr<RdmaQueuePair> q, bool f){
	ScheduleDecreaseRateMlx(q, 0, f);
...
/** BiCC **/
void RdmaHw::CheckRateDecreaseBiCCRns(Ptr<RdmaQueuePair> q){
        ScheduleDecreaseRateBiCCRns(q, 0);
        if (q->BiCC.m_decrease_rns_arrived){
//		std::cout << "jkjkjkjkjk" << std::endl;
                #if PRINT_LOG
                printf("%lu rate dec: %08x %08x %u %u (%0.3lf %.3lf)->", Simulator::Now().GetTimeStep(), q->sip.Get(), q->dip.Get(), q->sport, q->dport, q->mlx.m_targetRate.GetBitRate() * 1e-9, q->m_rate.GetBitRate() * 1e-9);
                #endif
                bool clamp = true;
...
void RdmaHw::ScheduleDecreaseRateBiCCRns(Ptr<RdmaQueuePair> q, uint32_t delta){
	q->BiCC.m_eventDecreaseRateRns = Simulator::Schedule(MicroSeconds(m_rateDecreaseInterval) + NanoSeconds(delta), &RdmaHw::CheckRateDecreaseBiCCRns, this, q);	
}
/** BiCC **/

void RdmaHw::ScheduleDecreaseRateMlx(Ptr<RdmaQueuePair> q, uint32_t delta, bool f){
	q->mlx.m_eventDecreaseRate = Simulator::Schedule(MicroSeconds(m_rateDecreaseInterval) + NanoSeconds(delta), &RdmaHw::CheckRateDecreaseMlx, this, q, f);
}
...
//	q->m_rate = q->m_max_rate/2;
//	std::cout << q->m_max_rate << std::endl;
