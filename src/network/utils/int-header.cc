...
uint32_t IntHeader::GetStaticSize(){
	if (mode == NORMAL){
		return sizeof(hop) + sizeof(nhop) + sizeof(ts) + sizeof(recvBiCCBytes); // + sizeof(isLongLoop);
	}else if (mode == TS){
		return sizeof(hop) + sizeof(nhop) + sizeof(ts) + sizeof(recvBiCCBytes); // + sizeof(isLongLoop);
	}else if (mode == PINT){
		return sizeof(pint);
...
void IntHeader::Serialize (Buffer::Iterator start) const{
	...
	if (mode == NORMAL){
		...
		i.WriteU64(ts);
		i.WriteU64(recvBiCCBytes);
	//	i.WriteU16(isLongLoop);
	//	i.WriteU8(CNP_type);
	}else if (mode == TS){
                for (uint32_t j = 0; j < maxHop; j++){
                        i.WriteU32(hop[j].buf[0]);
...
uint32_t IntHeader::Deserialize (Buffer::Iterator start){
	...
	if (mode == NORMAL){
		...
		ts = i.ReadU64();
		recvBiCCBytes = i.ReadU64();
	//	isLongLoop = i.ReadU16();
	//	CNP_type = i.ReadU8(); 
	}else if (mode == TS){
                for (uint32_t j = 0; j < maxHop; j++){
                        hop[j].buf[0] = i.ReadU32();
